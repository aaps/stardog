...
class HUD:
	...
	def draw(self, surface):
		...
		if self.game.debug:
			...
			if hasattr(self.game.player.pos, 'oldx'):
				dx = (self.game.player.pos.x - self.game.player.pos.oldx) / self.game.dt
				dy = (self.game.player.pos.y - self.game.player.pos.oldy) / self.game.dt
				print 'dx: %.1f dy: %.1f' % (dx, dy)
			self.game.player.pos.oldx, self.game.player.pos.oldy = self.game.player.pos.x, self.game.player.pos.y
		self.image.fill((0, 0, 0, 0))
		#TODO: don't hard-code this key:
		self.drawRadar(surface, self.game.keys[K_TAB])
		...
		pygame.draw.rect(self.image, (0, 50, 230), (x, y \
			+ h - h * self.game.player.pos.energy / self.game.player.pos.maxEnergy, 5, h \
			* self.game.player.pos.energy / self.game.player.pos.maxEnergy)) # full bar
		...
		pygame.draw.rect(self.image, (0, 180, 80), \
			(x, y + h - h * self.game.player.pos.xp / self.game.player.pos.next(), 5, \
			h * self.game.player.pos.xp / self.game.player.pos.next())) # full bar
		if(fontModule) and self.game.player.pos.developmentPoints:
			self.image.blit(FONT.render(str(self.game.player.pos.developmentPoints), \
						False, (0, 180, 80)), (x, y - 20))

		#edge warning:
		if self.game.player.pos.game.curSystem.drawEdgeWarning:
	...
	def drawRadar(self, surface, game, big = False):
		if big:
			...
			sunPos = (int(center[0] + (self.game.curSystem.sun.x - self.game.player.pos.x) / scale),
			int(center[1] + (self.game.curSystem.sun.y - self.game.player.pos.y) / scale))
		...
		for floater in self.game.curSystem.floaters:
			dotPos = int(center[0] + limit(-radius, \
					(floater.x - self.game.player.pos.x) / scale, radius)), \
					int(center[1] + limit(-radius, \
					(floater.y - self.game.player.pos.y) / scale, radius))
					...
					if isinstance(floater, Ship):#direction arrow
						...
						if hasattr(floater, 'goal'):
							p2 = (int(center[0] + limit(-radius,
								(floater.goal[0] - self.game.player.pos.x) / scale, radius)),
								int(center[1] + limit(-radius,
								(floater.goal[1] - self.game.player.pos.y) / scale, radius)))
...
# 	def draw(self, surface, self.game.player.pos):
# 		depth = 10
# 		for layer in self.bg:
# 			offset = self.game.player.pos.x / depth % 800, thisShip.y / depth % 800
