...
class Fitter(Facility):
	...
	# a list of parts and the fitting efficiency that makes up the price of fitting something
...
class Smelter(Facility):
	...
	# update
	# look if there is ore of scrap on planet or in a shop on planet and get / buy it, if the price is right
...
class TradeTerminal(Facility):
	...
	# player reserve a good on the list you want to buy sell (you are on the mission)
	# 
...
class Repair(Facility):
	...
	# the price will be dependable on repair efficiency and the amount of repair shops in the neighborhood
