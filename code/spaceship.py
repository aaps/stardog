...
def makeJuggernaut(game, pos, delta, direction=27, color = SUPER_WHITE, name=("Shippy","mcShipperson"), player=False, partlim=8):
    ...
    shield = BigShield(game.universe)
    for part in [gyro, generator, battery, cockpit, gun, gun2,
                 engine, engine2, quarter, shield]:
        if rand() > .8:
            addAdjective(part)
            if rand() > .6:
                addAdjective(part)
...
def playerShip(game, pos, delta, direction=270, \
    ...
    if type == 'destroyer':
        ship = makeDestroyer(game, pos, delta, direction, color, name,
                             player=True, partlim=12)
    elif type == 'freighter':
        ship = makeFreighter(game, pos, delta, direction, color, name,
                             player=True, partlim=15)
    elif type == 'interceptor':
        ship = makeInterceptor(game, pos, delta, direction, color, name,
                             player=True, partlim=10)
    elif type == 'juggernaut':
        ship = makeJuggernaut(game, pos, delta, direction, color, name,
                             player=True, partlim=10)
    elif type == 'scout':
        ship = makeScout(game, pos, delta, direction, color, name,
                             player=True, partlim=6)
    elif type == 'fighter':
        ship = makeFighter(game, pos, delta, direction, color, name,
                             player=True, partlim=6)
...
class Ship(Floater, Controllable):
    ...
    def __init__(self, game, pos, delta, direction = 270, \
        ...
        self.functions = [self.forward, self.reverse, self.left, self.right,
                          self.turnLeft, self.turnRight, self.shoot,
                          self.launchMissiles, self.launchMines,
                          self.toggleGatewayFocus, self.toggleRadar]
